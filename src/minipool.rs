    ...
    P: FnOnce() -> Result<R, Error>,
{
    ...
    std::thread::scope(move |scope| {
        ...
        let handles = std::iter::repeat(thread)
            .enumerate()
            .take(num_threads.get().into())
            .map(move |(n, thread)| {
                std::thread::Builder::new()
                    .name(format!("{name}{n}"))
                    .spawn_scoped(scope, thread)
            })
        ...
        handles
            .into_iter()
            .try_for_each(|h| h.join().map_err(|_| Error::ThreadSend)?)?;
        res
    })
}
