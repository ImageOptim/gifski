...
impl SettingsExt {
    pub(crate) fn gifsicle_loss(&self) -> u32 {
        if cfg!(feature = "gifsicle") && self.giflossy_quality < 100 {
            (100. / 5. - f32::from(self.giflossy_quality) / 5.)
                .powf(1.8)
                .ceil() as u32
                + 10
        } else {
...
fn dither_image(mut image: ImgRefMut<RGBA8>) {
    ...
    for (y, (row, aa)) in image
        .rows_mut()
        .zip(anti_aliasing.chunks_exact(width))
        .enumerate()
    {
...
impl Writer {
    ...
    fn write_inner(
        ...
        let resize_thread = thread::Builder::new()
            .name("resize".into())
            .spawn(move || Self::make_resize(decode_queue_recv, diff_queue, &settings_ext))?;
        let (quant_queue, quant_queue_recv) = crossbeam_channel::bounded(0);
        let diff_thread = thread::Builder::new()
            .name("diff".into())
            .spawn(move || Self::make_diffs(diff_queue_recv, quant_queue, &settings_ext))?;
        let (remap_queue, remap_queue_recv) = ordqueue::new(0);
        let quant_thread = thread::Builder::new()
            .name("quant".into())
            .spawn(move || Self::quantize_frames(quant_queue_recv, remap_queue, &settings_ext))?;
